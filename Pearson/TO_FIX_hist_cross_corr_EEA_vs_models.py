...
parser = argparse.ArgumentParser(description='Cross-Correlation EEA - CAMS EU - GEOS CF - CAMS GLOBALE')
...
parser.add_argument('-lag', '--lag', help='Lag of Cross-Correlation', type=int, required=True)
...
# Values of Cross-correlation lag [-lag,lag]
# It is removed the lag = 0
list_lags = [*range(-int(args["lag"]), int(args["lag"])+1, 2)]
list_lags.remove(0)
...
# -------------- Histograms of Cross-Correlations --------------
...
def compute_cross_correlation(sublist_eea_station, y, method_corr, windows_lenght, lag):

    list_cross_corr = []

    # For each partition of time series
    for i in range(len(sublist_eea_station)):

        current_sublist_eea = sublist_eea_station[i]
        current_sublist_y = []

        idx_y_start = i + lag

        # If we are in the head of time series
        if idx_y_start < 0:

            for k in range(idx_y_start):
                current_sublist_y.append(0.0)

            for k in range(0, windows_lenght - idx_y_start):
                current_sublist_y.append(y[k])
        
        # If we are in the last part of time series
        if idx_y_start >= len(y):
            
            for k in range(windows_lenght):
                current_sublist_y.append(0.0)

            for k in range(i, len(y)):
                current_sublist_y.append(y[k])
            
            elem_to_add = windows_lenght - (len(y) - i)

            for k in range(elem_to_add):
                current_sublist_y.append(0.0)
        
        else:
            for k in range(i, i+windows_lenght):
        ...
        x_pd = pd.Series(current_sublist_eea)
        y_pd = pd.Series(current_sublist_y)

        corss_corr_current_sublist = x_pd.corr(y_pd, method=method_corr)
        ...
        if math.isnan(corss_corr_current_sublist):
            corss_corr_current_sublist = 0.0

        list_cross_corr.append(abs(corss_corr_current_sublist))
    
    return list_cross_corr
...
PATH_DIR_PLOTS = joinpath(PATH_DIR_PLOTS, "EEA_hist_cross_corr_all_air_model_" + str(model_level_air_pollution) + "_pm_" + str(model_level_pm) + "_camsEU_" + str(list_numeric_model_cams_eu[idx_numeric_model_cams_eu]))
...
for cod_station in list_cod_stations:
    ...
    for windows_lenght in list_windows_lenght:
        ...
        for lag in list_lags:
            ...
            # Compute Cross-correlation between EEA vs CAMS Global
            list_cross_corr_EEA_vs_cams_global = []

            if not_available_cams_global == False:
                list_cross_corr_EEA_vs_cams_global = compute_cross_correlation(    
            ...
            # Compute Cross-correlation between EEA vs Geos CF
            list_cross_corr_EEA_vs_geos_cf = []

            if not_available_goes_cf == False:
                list_cross_corr_EEA_vs_geos_cf = compute_cross_correlation( 
                                                                            sublists_EEA_station, 
                                                                            dict_values_geos_cf[cod_station], 
                                                                            method_corr,
                                                                            windows_lenght,
            ...
            # Compute Cross-correlation between EEA vs CAMS Europe
            list_cross_corr_EEA_vs_cams_eu = []

            if not_available_cams_eu == False:
                list_cross_corr_EEA_vs_cams_eu = compute_cross_correlation(    
                                                                            sublists_EEA_station, 
                                                                            dict_values_cams_eu[cod_station], 
                                                                            method_corr,
                                                                            windows_lenght,
            ...
            # Compute Cross-correlation between CAMS Europe vs GEOS CF
            list_cross_corr_cams_eu_vs_geos_cf = []

            if not_available_cams_eu == False and not_available_goes_cf == False:
                list_cross_corr_cams_eu_vs_geos_cf = compute_cross_correlation(    
                                                                            sublists_cams_eu, 
                                                                            dict_values_geos_cf[cod_station], 
                                                                            method_corr,
                                                                            windows_lenght,
            ...
            # Compute Cross-correlation between CAMS Europe vs CAMS Global
            list_cross_corr_cams_eu_vs_cams_global = []

            if not_available_cams_eu == False and not_available_cams_global == False:
                list_cross_corr_cams_eu_vs_cams_global = compute_cross_correlation(    
                                                                                sublists_cams_eu, 
                                                                                dict_values_cams_global[cod_station], 
                                                                                method_corr,
                                                                                windows_lenght,
            ...
            # Compute Cross-correlation between GEOS CF vs CAMS Global
            list_cross_corr_geos_cf_vs_cams_global = []

            if not_available_goes_cf == False and not_available_cams_global == False:
                list_cross_corr_geos_cf_vs_cams_global = compute_cross_correlation(    
                                                                                sublists_geos_cf, 
                                                                                dict_values_cams_global[cod_station], 
                                                                                method_corr,
                                                                                windows_lenght,
            ...
            PATH_DIR_PLOTS_current = joinpath(PATH_DIR_PLOTS_current, "Lag_" + str(lag) + "_WL_" + str(windows_lenght))

            if not os.path.exists(PATH_DIR_PLOTS_current):
                os.mkdir(PATH_DIR_PLOTS_current)

            plot_hist_EEA(  
                            cod_station, year_to_consider, windows_lenght, lag, \
                            method_corr, is_previous_elems, \
                            list_cross_corr_EEA_vs_cams_global, list_cross_corr_EEA_vs_geos_cf, \
                            list_cross_corr_EEA_vs_cams_eu, air_poll_selected, PATH_DIR_PLOTS_current
                    )
            ...
            plot_hist_models(  
                                ...
                                method_corr, is_previous_elems, \
                                list_cross_corr_cams_eu_vs_geos_cf, list_cross_corr_cams_eu_vs_cams_global, \
                                list_cross_corr_geos_cf_vs_cams_global, air_poll_selected, PATH_DIR_PLOTS_current
                    )