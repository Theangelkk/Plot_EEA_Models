...
# ------------ INFORMAZIONI SU CAMS EUROPA ------------
not_available_cams_eu = False

if air_pollutant_selected == "PM2p5" or air_pollutant_selected == "PM10":
    DATADIR_CAMS_EU = joinpath(os.getcwd(), "CAMS_Europa_model_level_" + str(model_level_pm))
else:
    DATADIR_CAMS_EU = joinpath(os.getcwd(), "CAMS_Europa_model_level_" + str(model_level_air_pollution))
...
# Risoluzione temporale di 1 ora
time_res_cams_eu = 1
...
def compute_all_similarity(np_1, np_2):
    
    # It cannot be computed the PSNR and SSIM

    print("MSE: ", mse(np_1, np_2))
    print("RMSE: ", rmse(np_1, np_2))
    print("UQI: ", uqi(np_1, np_2))
    print("ERGAS: ", ergas(np_1, np_2))
    print("SCC: ", scc(np_1, np_2))
    print("RASE: ", rase(np_1, np_2))
    print("SAM: ", sam(np_1, np_2))
    print("VIF: ", vifp(np_1, np_2))

    print("Correlation: ", np.corrcoef(np_1, np_2)[0,1])

    print("Percentage of change: ", np.sum((((np_1 - np_2) * 100) / np_1)) / (np_1.shape[0] * np_1.shape[1]))
    print("Percentage of change abs: ", np.sum(np.abs(((np_1 - np_2) * 100) / np_1)) / (np_1.shape[0] * np_1.shape[1]))
...
for time in range(diff_dates_hours):
    ...
    print(np_current_date_air_pol_cams_eu.shape)
    print(np_current_date_air_pol_geos_cf.shape)
    print(np_current_date_air_pol_cams_global.shape)
    ...
    print(np_current_date_air_pol_geos_cf_cams_global.shape)
    ...
    list_air_pollutant_ds = [
                                ds_current_date_cams_global.to_array(), 
                                np_current_date_air_pol_geos_cf_cams_global,
                                np_current_date_air_pol_cams_eu_cams_global
                            ]
    
    list_idx_lat.append(ds_current_date_cams_global.indexes["latitude"])
    list_idx_lon.append(ds_current_date_cams_global.indexes["longitude"])

    #list_idx_lat.append(ds_current_date_cams_global.indexes["lat"])
    #list_idx_lon.append(ds_current_date_cams_global.indexes["lon"])
    
    #plot_heatmap(lon_italy_bnds, lat_italy_bnds, list_idx_lat, list_idx_lon, list_air_pollutant_ds, max_value_dict, title, savefig = False, path_fig=path_img, dpi=300)

    # -------------------- Similarity --------------------
    # Link: https://towardsdatascience.com/measuring-similarity-in-two-images-using-python-b72233eb53c6

    # CAMS EU vs CAMS Global
    compute_all_similarity(np_current_date_air_pol_cams_eu_cams_global, np_current_date_air_pol_cams_global)
